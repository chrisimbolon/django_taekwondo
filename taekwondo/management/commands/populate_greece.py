from django.core.management.base import BaseCommand
from taekwondo.models import Province, City

class Command(BaseCommand):
    help = "Populate Greece's regions and major cities"

    def handle(self, *args, **kwargs):
        country = "GR"  # Greece's country code

        greece_data = {
            "Attica": ["Athens", "Piraeus"],
            "Central Macedonia": ["Thessaloniki", "Serres", "Katerini"],
            "Crete": ["Heraklion", "Chania", "Rethymno"],
            "Thessaly": ["Larissa", "Volos", "Trikala"],
            "Peloponnese": ["Kalamata", "Tripoli", "Corinth"],
            "Western Greece": ["Patras", "Agrinio", "Pyrgos"],
            "Eastern Macedonia and Thrace": ["Kavala", "Xanthi", "Komotini"],
        }

        for region, cities in greece_data.items():
            province, created = Province.objects.get_or_create(province_name=region, country=country)
            if created:
                self.stdout.write(self.style.SUCCESS(f"Province '{region}' added successfully!"))
            for city in cities:
                city_obj, city_created = City.objects.get_or_create(city_name=city, province=province)
                if city_created:
                    self.stdout.write(self.style.SUCCESS(f"City '{city}' added under '{region}'!"))

        self.stdout.write(self.style.SUCCESS("Database successfully populated with Greece's regions and cities!"))
